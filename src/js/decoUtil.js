...
function getAssembledData(keywords, customDefaultStyle, isCaseSensitive) {
    ...
    keywords.forEach((v) => {
        ...
        result[text] = Object.assign({}, defaultStyle, customDefaultStyle, v);
    });

    Object.keys(DEFAULT_KEYWORDS).forEach((v) => {
        if (!result[v]) {
            result[v] = Object.assign({}, defaultStyle, customDefaultStyle, DEFAULT_KEYWORDS[v]);
        }
    });
...
function searchAnnotations(workspaceState, pattern, callback) {
    ...
    var settings = workspace.getConfiguration('blinkhubScanner');
...
function annotationsFound(err, annotations, annotationList) {
    if (err) {
        console.log('blinkhubScanner err:', err);
...
function createStatusBarItem() {
    ...
    statusBarItem.command = 'blinkhubScanner.showOutputChannel';
