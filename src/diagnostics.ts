  ...
  for (let lineIndex = 0; lineIndex < doc.lineCount; lineIndex++) {
    ...
    for (let i = 0; i < VALIDATOR_REGEXPS_KEYS.length; i++) {
      ...
      if (
        ...
        key === "POSTAL_CODE_BR" ||
        key === "POSTAL_CODE_FR" ||
        key === "PROPERTY_ADDRESS_AU" ||
        key === "PROPERTY_ADDRESS_UK"
      ) {
        if (REGEX_NEW.test(strToTest)) {
          if (text !== "") {
            ...
            childToDo = new DiagnosticsContent(
              toDoMesg,
              lineIndex,
              toDoErrorLevel.toLowerCase(),
              s,
              toDoError,
              key
            );
      ...
      if (childToDo === undefined) {
        continue;
      }

      let diagnostic: vscode.Diagnostic = createDiagnostic(
        lineOfText,
        childToDo,
        lineIndex
      );
...
function createDiagnostic(
  ...
  const range = new vscode.Range(
    ...
    index,
    lineIndex,
    index + childToDo.lineContent.length
  );
  let message = "";
