...
import { COMMANDS, EXTENSION_ID, REGEX, TODO, VIEWS, WELCOMEMSG, DASHBOARDURL, BLINKHUB_CONTEXT } from './constants';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let isCaseSensitive: any, assembledData: { [x: string]: any; }, decorationTypes: { [x: string]: vscode.TextEditorDecorationType; }, pattern: string | RegExp, styleForRegExp: vscode.DecorationRenderOptions, keywordsPattern: string;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("blinkhubScanner.dashboard", () => {
       vscode.env.openExternal(vscode.Uri.parse(DASHBOARDURL + "?token=" + TokenManager.getToken()));
    })
  );
    ...
    vscode.commands.registerCommand("blinkhubScanner.authenticate", () => {
      try {
        authenticate(async function(){
          ...
          await vscode.commands.executeCommand('setContext', BLINKHUB_CONTEXT.LOGGEDIN, true);
          vscode.window.registerTreeDataProvider(VIEWS.TODO_LIST, todoTreeList);
          vscode.window.registerTreeDataProvider(VIEWS.TODO_REGULATORY, regulatory);
          ...
          const emojiDiagnostics = vscode.languages.createDiagnosticCollection("blinkhubScanner");
          ...
          context.subscriptions.push(vscode.commands.registerCommand('blinkhubScanner.showOutputChannel', function () {
  ...
  function init(settings:any) {
    ...
    if (keywordsPattern.trim()) {
        styleForRegExp = Object.assign({}, util.defaultStyle, customDefaultStyle, {
  ...
  function updateDecorations() {
    ...
    Object.keys(decorationTypes).forEach((v) => {
          ...
          if (editor !== undefined) {editor.setDecorations(decorationType, rangeOption);}
      });
  }
 ...
 */
 export class Emojizer implements vscode.CodeActionProvider {
  ...
  private async commentLine(url:string): Promise<void> {
    ...
    if (emoji === 'IGNORE'){
      fix = new vscode.CodeAction(`Ignore this suggestion (BlinkTrust AI)`, vscode.CodeActionKind.QuickFix);
      
      fix.edit = new vscode.WorkspaceEdit();
      let editText = Delimiter.getDelimiter(document.languageId).replace("text", "@bt-ignore <please let us the reason to ignore>\r\n");
      let newRange: vscode.Range = new Range(range.start.line - 1, 0, range.start.line, editText.length) ;
		  fix.edit.insert(document.uri,  newRange.start, editText);
    }
    else{
      fix = new vscode.CodeAction(`Ignore suggestions for current file (BlinkTrust AI)`, vscode.CodeActionKind.QuickFix);
      let editText = Delimiter.getDelimiter(document.languageId).replace("text", "@bt-ignore-file <please let us the reason to ignore>\r\n");
	...
	private createCommand(): vscode.CodeAction {
		const action = new vscode.CodeAction('Show more details and suggestions (Blink Trust AI)', vscode.CodeActionKind.QuickFix);
 ...
 */
 export class Emojinfo implements vscode.CodeActionProvider {
	...
	private createCommandCodeAction(diagnostic: vscode.Diagnostic): vscode.CodeAction {
		const action = new vscode.CodeAction('Show more details and suggestions (Blink Trust AI)', vscode.CodeActionKind.QuickFix);
		action.command = { command: COMMAND, title: 'Learn more about fixmes', tooltip: 'This will open the unicode fixme page.' };
